...
Evernote is one of the few products that has stayed genuinely focused on note-taking. It has plenty to complain about, but a note is far more than a saved file — I have tried to switch away to many other note products and failed every time, so Evernote stays. At the same time it is clearly disconnected from today's agent ecosystem — and frankly, it doesn't *need* native AI at all.

I built [evernote-ai-sidebar](https://github.com/ghbhiee/evernote-ai-sidebar) to make that point: an AI panel docked right inside the Evernote desktop app, connecting a traditional tool and its data to AI from the outside. This is a pattern, not a one-off — users can wire their own classic tools up to AI this way, there is always a way in, and it can be perfectly convenient. Vendor-built AI features are not a prerequisite. Evernote has been pushing in that native-AI direction; I have never bought it. When I built the sidebar, Evernote had no MCP server yet. They have since shipped one — late, which tells you how much they would rather keep your data inside their own walls — and now any agent reaches this traditional store just fine (this very project talks to Evernote exclusively through that MCP). That is the real shape of the agent era: **everything is a tool for AI**. A notes app doesn't need to become an AI product; it needs to keep being a great notes app and let agents reach in.
