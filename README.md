...
纯文本 markdown 三端双向同步：文件系统（iCloud Drive）⇄ Apple Notes ⇄ Evernote。
Mac 做 hub（星型拓扑），git 做历史与三方合并，Swift 菜单栏 App 做守护与监控。

- 三端任一处增删改，其余两端几分钟内跟上；文件/git 永远存标准 md，写入笔记软件时渲染为富格式
- 冲突走 diff3 三方合并（行不重叠自动合并；同行冲突新者胜、败方全文留底），修改胜过删除
- 含图片/附件的笔记自动排除（不拉不推不删），永不丢内容
- 可选单向镜像到 Google Drive 文件夹

## 安装

```bash
git clone https://github.com/ghbhiee/notesync ~/notesync
cd ~/notesync && python3 -m notesync init      # 生成 ~/Documents/NoteSync/config.json
python3 -m unittest discover -s tests          # 可选：跑测试
cd macapp && ./build.sh                        # 构建菜单栏 App（可设 CODESIGN_ID/BUNDLE_ID）
cp -R NoteSyncBar.app /Applications/ && open /Applications/NoteSyncBar.app
```

前置：macOS 13+、Xcode 命令行工具、Node（Evernote 走官方 MCP，引擎 spawn `npx mcp-remote`，首次需在任一 MCP 客户端完成 Evernote OAuth，凭据共享于 `~/.mcp-auth`）。首次启动 App 时允许它访问「文稿」与 iCloud Drive。配置见 `config.example.json`（`engine_dir` 指向你的 clone 位置）；App 菜单里也有设置界面。

技术细节见 [DESIGN.md](DESIGN.md)。以下为开发状态记录。
